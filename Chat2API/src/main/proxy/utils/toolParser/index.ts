...
export function parseToolCallsStream(
  ...
  console.log('[parseToolCallsStream] Received chunk length:', content.length, 'Buffer size before:', state.contentBuffer.length)
  state.contentBuffer += content
  console.log('[parseToolCallsStream] Buffer size after:', state.contentBuffer.length, 'isBufferingToolCall:', state.isBufferingToolCall)
  ...
  if (!state.isBufferingToolCall) {
    ...
    if (markerIdx !== -1) {
      ...
      for (let i = 0; i < state.contentBuffer.length; i++) {
        if (state.contentBuffer[i] === '[') {
          ...
          if (marker.startsWith(potentialMarker)) {
            // It MIGHT be the start of [function_calls]
            foundPartial = true
            if (i > 0) {
      ...
      if (foundPartial) {
        // We found a partial marker, wait for more data, but DO NOT set isBufferingToolCall yet
        // isBufferingToolCall means we have definitively found [function_calls]
        return { chunks: result, shouldFlush: false }
      }
    }
  }
  ...
  if (state.isBufferingToolCall) {
    // 检查缓冲区中是否包含结束标签
    const hasEndTag = state.contentBuffer.includes('[/function_calls]')
    // 如果大模型返回了残缺的闭合标签，比如 `[/function_calls` 还没闭合，我们需要稍微宽容一点，
    // 或者我们直接用正则来找结束标签，包括可能不带 `]` 的情况，以防网络断流导致一直阻塞。
    const hasPossibleEndTag = state.contentBuffer.includes('[/function_calls') || state.contentBuffer.includes('[/call]')
    
    if (hasEndTag || (hasPossibleEndTag && state.contentBuffer.length > 100)) {
      console.log('[parseToolCallsStream] End tag detected, attempting to parse. Buffer length:', state.contentBuffer.length)
      // 包含结束标签，表示整个工具调用块已经接收完毕，可以进行解析
      const { toolCalls } = parseToolCalls(state.contentBuffer)
      console.log('[parseToolCallsStream] Parsed tool calls count:', toolCalls.length)
      ...
      if (toolCalls.length > 0) {
        ...
        // Clean up empty function_calls blocks
        state.contentBuffer = state.contentBuffer.replace(/\[function_calls\]\s*\[\/function_calls\]/g, '')

        // Check if there is another [function_calls] block starting
        const nextMarkerIdx = state.contentBuffer.indexOf('[function_calls]')
        if (nextMarkerIdx !== -1) {
          // There is another block starting!
          state.isBufferingToolCall = true
          const textBefore = state.contentBuffer.substring(0, nextMarkerIdx).trim()
          if (textBefore) { 
             result.push({
              delta: {
                content: textBefore
          ...
          state.contentBuffer = state.contentBuffer.substring(nextMarkerIdx)
        } else {
          // No more tool calls starting
          state.isBufferingToolCall = false
          
          // Clean up any stray tags
          const remainingText = state.contentBuffer.replace(/\[\/?function_calls\]?/g, '').trim()
        ...
        // 关键修复：不要在这里提前发送 finish_reason: 'tool_calls'，
        // 因为外层的 stream.ts 或者 handler 也会在结束时发送 finish_reason，
        // 我们只需确保 parseToolCallsStream 输出的 chunk 可以让外层知道有 tool_calls。
      ...
      // 还没收到结束标签，继续缓冲，不输出任何东西
      // 增加缓冲区限制以支持大型工具调用（如写入大文件）
      // 从 5000 提升到 500000 (约 500KB)
      if (state.contentBuffer.length > 500000) {
        console.warn('[parseToolCallsStream] Buffer exceeded 500KB limit, flushing as content')
        state.isBufferingToolCall = false
        if (!state.hasEmittedToolCall) {
          result.push({
            delta: {
...
export function flushToolCallBuffer(
  ...
  console.log('[flushToolCallBuffer] state.contentBuffer =', JSON.stringify(state.contentBuffer))
  ...
  if (toolCalls.length > 0) {
    ...
    // 输出剩余内容
    const remainingText = cleanContent.replace(/\[\/?function_calls\]?/g, '').trim()
    if (remainingText) {
      result.push({
...
function parseBracketFormat(content: string): { content: string; toolCalls: ToolCall[]; rawMatches: string[] } {
  ...
  const blockRegex = /\[function_calls\]([\s\S]{0,50000}?)(?:\[\/function_calls\]|\[\/call\]\s*$|$)/g
...
function parseXmlFormat(content: string): { content: string; toolCalls: ToolCall[]; rawMatches: string[] } {
  ...
  const toolUseRegex = /<tool_use>\s*<name>([^<]+)<\/name>\s*<arguments>([\s\S]{0,50000}?)<\/arguments>\s*<\/tool_use>/g
...
function parseAnthropicFormat(content: string): { content: string; toolCalls: ToolCall[]; rawMatches: string[] } {
  ...
  const blockRegex = /<antml:function_calls>\s*([\s\S]{0,50000}?)\s*<\/antml:function_calls>/g
  let blockMatch
  ...
  while ((blockMatch = blockRegex.exec(content)) !== null) {
    ...
    const invokeRegex = /<antml:invoke name="([^"]+)">\s*<antml:parameters>([\s\S]{0,50000}?)<\/antml:parameters>\s*<\/antml:invoke>/g
    let invokeMatch
    let index = 0
...
function parseJsonFormat(content: string): { content: string; toolCalls: ToolCall[]; rawMatches: string[] } {
  ...
  const toolCallsRegex = /"tool_calls"\s*:\s*\[([\s\S]{0,50000}?)\]/g
  let match
