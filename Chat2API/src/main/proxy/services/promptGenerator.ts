 ...
 */
function generateToolWrapHint(format: ProtocolFormat = 'bracket'): string {
  if (format === 'xml') {
    return `

IMPORTANT: If you need to use a tool, you MUST wrap the tool call inside a <tool_use> block exactly like:
<tool_use>
...
- Start with <tool_use>
- Include <name> and <arguments> tags
...
export class PromptGenerator {
  ...
  static generate(tools: ChatCompletionTool[], options: PromptGenerationOptions): string {
    ...
    if (customTemplate && customTemplate.trim().length > 0) {
      ...
      let result = substituteTemplateVariables(customTemplate, variables)
      
      // If the custom template didn't contain {{tools}}, append it automatically to avoid breaking tool calls
      if (!customTemplate.includes('{{tools}}')) {
        console.warn('[PromptGenerator] Custom template missing {{tools}} variable, appending definitions automatically')
        result += `\n\n## Available Tools\n${variables.tools}\n\n${variables.format}`
      }
      return result
    }
   ...
   */
  static generateWrapHint(format: ProtocolFormat = 'bracket'): string {
    return generateToolWrapHint(format)
  }
