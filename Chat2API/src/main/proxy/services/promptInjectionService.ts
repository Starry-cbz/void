...
export class PromptInjectionService {
  ...
  process(
    ...
    const injectedMessages = this.injectToMessages(messages, prompt, config.defaultFormat)
  ...
  private shouldInject(
    ...
    if (config.mode === 'auto') {
      // Known client - check if it actually injects prompt
      if (detection.isKnownClient) {
        if (detection.injectsPrompt) {
          return { shouldInject: false, reason: `known_client_${detection.clientType}` }
        }
        // If known client doesn't inject prompt, we should inject
        return { shouldInject: true, reason: `known_client_needs_prompt_${detection.clientType}` }
      }
   ...
   * Inject prompt to messages
   */
  private injectToMessages(messages: ChatMessage[], prompt: string, format: ProtocolFormat): ChatMessage[] {
    if (!prompt) {
      return messages
    }
    ...
    // Also append the wrap hint to the last user message to prevent the model from forgetting
    // the tool call format in long conversations
    const wrapHint = PromptGenerator.generateWrapHint(format)
    for (let i = result.length - 1; i >= 0; i--) {
      if (result[i].role === 'user') {
        const currentContent = result[i].content
        if (typeof currentContent === 'string') {
          result[i] = { ...result[i], content: currentContent + wrapHint }
        } else if (Array.isArray(currentContent)) {
          result[i] = {
            ...result[i],
            content: [...currentContent, { type: 'text', text: wrapHint }],
          }
        }
        break
      }
