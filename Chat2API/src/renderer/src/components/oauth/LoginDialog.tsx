...
import { ExternalLink, AlertCircle, ListSingle } from 'lucide-react'
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
          ...
          <TabsContent value="manual" className="mt-4 space-y-4">
            {(canBatchMode || (config?.manualTokenConfigs.length && config.manualTokenConfigs.length > 1 && !isMiniMax)) && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                {canBatchMode && (
                  ...
                  >
                    <ListSingle className="mr-1 h-3.5 w-3.5" />
                    {batchMode ? t('oauth.singleMode') : t('oauth.batchMode')}
                  </Button>
                )}
              </div>
            )}
            ...
            {currentTokenConfig && (
              <TokenInput
                ...
                placeholder={batchMode && canBatchMode ? t('oauth.batchDescription') : t(currentTokenConfig.placeholderKey)}
                description={t(currentTokenConfig.descriptionKey)}
                helpUrl={currentTokenConfig.helpUrl}
                value={token}
                onChange={setToken}
                onSubmit={!batchMode ? handleManualSubmit : undefined}
                disabled={isLoading}
                error={error}
                batchMode={batchMode && canBatchMode}
              />
            )}
            ...
            {batchResults && (
              <div className={`rounded-lg p-3 text-sm ${batchResults.failed === 0 ? 'bg-green-50 text-green-700' : batchResults.success === 0 ? 'bg-red-50 text-red-700' : 'bg-yellow-50 text-yellow-700'}`}>
                <p className="font-medium">{t('oauth.batchComplete')}</p>
                <p className="mt-1">
                  {t('oauth.batchCompleteDesc', { success: batchResults.success, failed: batchResults.failed })}
                </p>
              </div>
            )}
        ...
        <DialogFooter className="mt-4">
          ...
          {activeTab === 'manual' && (
            ...
            >
              {isLoading ? (
                batchMode && canBatchMode ? t('oauth.batchProcessing') : t('oauth.validating')
              ) : (
                batchMode && canBatchMode ? t('oauth.batchMode') : t('oauth.confirmLogin')
              )}
            </Button>
          )}
        </DialogFooter>
