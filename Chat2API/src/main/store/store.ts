...
class StoreManager {
  ...
  private logSaveTimeout: NodeJS.Timeout | null = null
  private pendingLogSave: boolean = false
  ...
  private scheduleLogSave(logs: LogEntry[]): void {
    if (this.pendingLogSave) {
      return
    }
    
    if (this.logSaveTimeout) {
      clearTimeout(this.logSaveTimeout)
    }
    
    this.logSaveTimeout = setTimeout(() => {
      this.pendingLogSave = true
      try {
        this.store!.set('logs', logs)
      } finally {
        this.pendingLogSave = false
