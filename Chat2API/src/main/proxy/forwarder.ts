...
export class RequestForwarder {
  ...
  private transformRequestForPromptToolUse(
    ...
    provider?: Provider
  ): { messages: any[]; tools: ChatCompletionTool[] | undefined; shouldParseToolCalls: boolean } {
    const { messages, tools, model } = request

    // 1. Quick check: native function calling model
    if (isNativeFunctionCallingModel(model)) {
      return { messages, tools, shouldParseToolCalls: false } // Native handles its own parsing
    }
    ...
    return { messages: result.messages, tools: undefined, shouldParseToolCalls: result.shouldParseToolCalls }
  }
...
3. Do NOT output any other text before or after the [function_calls] block`
  ...
  private async forwardQwen(
    ...
    try {
      ...
      const handler = new QwenStreamHandler(actualModel, deleteSessionCallback, transformed.shouldParseToolCalls)
