 ...
 * Extracts tool calls from XML-style tags and forwards Bracket style to unified parser
 */
export function parseToolUse(content: string): ToolCall[] {
  // Use the new unified parser which handles bracket format natively
  const { parseToolCalls } = require('./utils/toolParser/index')
  const { toolCalls } = parseToolCalls(content)
  
  // Also try the legacy XML format regex just in case
  if (toolCalls.length === 0) {
    const legacyToolCalls: ToolCall[] = []
  ...
  // Format to legacy ToolCall type
...
export function hasToolUse(content: string): boolean {
  return /<?tool_use>/i.test(content) || content.includes('[function_calls]') || /\[call[:=]/.test(content)
}
