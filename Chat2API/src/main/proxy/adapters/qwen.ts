...
export class QwenAdapter {
  ...
  // Global Context Hash Mapping: Hash -> { sessionId, parentReqId }
  private static contextMap: Map<string, { sessionId: string; parentReqId: string }> = new Map()
  ...
  static generateContextHash(messages: any[]): string {
    ...
    const historyString = JSON.stringify(filteredMessages.map(m => {
      let contentStr = extractTextContent(m.content)
  ...
  private calculateContextHash(messages: any[]): string {
    return QwenAdapter.generateContextHash(messages)
  }
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{
    ...
    let sessionId = uuid(false)
    let parentReqId = '0'
    let sceneParam = 'first_turn'
    ...
    if (request.messages && request.messages.length > 1) {
      ...
      const historyPrefix = lastAssistantIndex !== -1 
        ? request.messages.slice(0, lastAssistantIndex + 1)
        : request.messages.slice(0, request.messages.length - 1)
      contextHash = this.calculateContextHash(historyPrefix)
      const mappedContext = QwenAdapter.contextMap.get(contextHash)
      
      if (mappedContext) {
        sessionId = mappedContext.sessionId
        parentReqId = mappedContext.parentReqId
        sceneParam = 'next_turn'
        isContinuation = true
        console.log(`[Qwen] Context matched! Continuing session ${sessionId} from parent ${parentReqId}`)
      } else {
        console.log(`[Qwen] Context hash not found, starting a new session. Hash: ${contextHash}`)
      }
    } else {
      console.log(`[Qwen] First turn dialogue.`)
    }
    ...
    // Build content depending on continuation state
    let finalContent = ''
    ...
    if (isContinuation) {
      ...
      const newMessages = lastAssistantIndex !== -1
        ? request.messages.slice(lastAssistantIndex + 1)
      ...
      // We also add tools prompt if needed for this turn
      if (request.tools && request.tools.length > 0 && !hasToolPromptInjected(newMessages)) {
        finalContent += TOOL_WRAP_HINT
      }
    } else {
      // Find system message and user message (historical concatenation for first turn / unmapped context)
      let systemPrompt = ''
      let userContent = ''
      ...
      for (const msg of request.messages) {
        if (msg.role === 'system') {
          ...
          if ((msg as any).tool_calls && Array.isArray((msg as any).tool_calls) && (msg as any).tool_calls.length > 0) {
            const toolCallsText = (msg as any).tool_calls.map((tc: any) => 
    ...
    return { response, sessionId, reqId, contextHash }
  }
  ...
  static recordNextContextMap(messages: any[], assistantMessage: any, sessionId: string, reqId: string) {
    ...
    const nextContextHash = QwenAdapter.generateContextHash(newMessages)
    
    QwenAdapter.contextMap.set(nextContextHash, { sessionId, parentReqId: reqId })
    console.log(`[Qwen] Registered next context hash: ${nextContextHash} -> session: ${sessionId}, reqId: ${reqId}`)
    
    // Prevent map from growing infinitely
    if (QwenAdapter.contextMap.size > 1000) {
      const firstKey = QwenAdapter.contextMap.keys().next().value
      if (firstKey) QwenAdapter.contextMap.delete(firstKey)
    }
  }
}
...
export class QwenStreamHandler {
  ...
  constructor(model: string, onEnd?: (sessionId: string) => void, shouldParseToolCalls: boolean = true) {
    this.model = model
        ...
        if (eventData && eventData !== '[DONE]') {
          try {
            ...
            if (result.communication) {
              if (!this.sessionId && (result.communication.sessionid || result.communication.session_id)) {
                this.sessionId = result.communication.sessionid || result.communication.session_id
              }
              if (!this.responseId && (result.communication.reqid || result.communication.req_id)) {
                this.responseId = result.communication.reqid || result.communication.req_id
              }
            }
                ...
                if ((msg.mime_type === 'text/plain' || msg.mime_type === 'multi_load/iframe') && msg.content) {
                  ...
                  if (newContent.length > this.content.length) {
                    ...
                    if (!this.shouldParseToolCalls) {
                      ...
                      transStream.write(`data: ${JSON.stringify({
                        ...baseChunk,
                        choices: [{ index: 0, delta: { role: !this.sentRole ? 'assistant' : undefined, content: chunk }, finish_reason: null }]
                      })}\n\n`)
                      this.sentRole = true
        ...
        while (true) {
          ...
          if (eventData && eventData !== '[DONE]') {
            try {
              ...
              if (result.communication) {
                if (!data.id && (result.communication.sessionid || result.communication.session_id)) {
                  data.id = result.communication.sessionid || result.communication.session_id
                  this.sessionId = result.communication.sessionid || result.communication.session_id
                }
                if (!this.responseId && (result.communication.reqid || result.communication.req_id)) {
                  this.responseId = result.communication.reqid || result.communication.req_id
                  ...
                  if (msg.status === 'complete' || msg.status === 'finished') {
                    if (msg.mime_type === 'multi_load/iframe') {
                      ...
                      if (this.shouldParseToolCalls) {
                        ...
                        data.choices[0].message.content = contentAccumulator.trim()
                      }
